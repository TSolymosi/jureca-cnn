...
class SpectralSpatialResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.spectral_conv = nn.Sequential(
            spectral_conv1d(1, 8, kernel_size=11, stride=4),
            nn.BatchNorm3d(8),
            nn.ReLU(inplace=True),
            spectral_conv1d(8, 16, kernel_size=11, stride=4),
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
            spectral_conv1d(16, block_inplanes[0], kernel_size=11, stride=4),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-scale-params", type=str, nargs='+', default=["Dens", "Lum", "radius"])
    ...
    train_loader, test_loader, dataset = create_dataloaders(
        ...
        log_scale_params=["Dens", "Lum", "radius"],
    )
    # Create folder for plots
    os.makedirs(f"ResNetPlots", exist_ok=True)
    # Create specific folder
    os.makedirs(f"ResNetPlots/All_files", exist_ok=True)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            for i in range(preds.shape[1]):
                ...
                plt.title(f"Epoch {epoch+1} - {labels[i]}")
                plt.grid(True)
                plt.tight_layout()
                plt.savefig(f"ResNetPlots/All_files/epoch_{epoch+1}_{labels[i]}.png")
                plt.close()
    ...
    # Save the model
    #torch.save(model.module.state_dict(), f"Weights/ResNet3D_{model_depth}_weights_2000.pth")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("ResNetPlots/All_files/Train_vs_Validation.png")
    print("Saved: Train_vs_Validation.png")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("ResNetPlots/All_files/per_parameter_mae.png")
    print("Saved: per_parameter_mae.png")
    ...
    for param in args.model_params:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"ResNetPlots/All_files/{param}_loss.png")
        print(f"Saved: ResNetPlots/{param}_loss.png")