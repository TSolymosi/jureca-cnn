...
class LossCurvesCallback(Callback):
    ...
    def on_fit_end(self, trainer, pl_module):
        ...
        if len(self.history["epoch"]) > 0:
            plt.figure()
            # Plot loss curves, but clip extreme values for better visibility
            plt.plot(self.history["epoch"], np.clip(self.history["train/loss"], None, 10), label="train/loss")
            
            plt.plot(self.history["epoch"], np.clip(self.history["val/loss"], None, 10), label="val/loss")
            plt.xlabel("Epoch")
            plt.ylabel("Loss")
