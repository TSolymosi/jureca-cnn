...
# -------------------------------------------------------------------------
# Replace your setup function with this version.
# This is the final and most robust implementation.
# -------------------------------------------------------------------------

def setup(rank, world_size):
    """
    Initializes the distributed process group robustly for HPC environments
    by explicitly resolving the master hostname to an IPv4 address.
    """
    
    username = getpass.getuser()
    job_id = os.environ.get("SLURM_JOB_ID", "local")
    temp_file_path = f"/tmp/{username}_{job_id}_master_addr.txt"

    if rank == 0:
        # Step 1: Get the fully qualified hostname.
        hostname = socket.getfqdn()
        
        # Step 2: Resolve this hostname to its IPv4 address. This is the crucial step.
        try:
            ip_address = socket.gethostbyname(hostname)
        except socket.gaierror:
            # Fallback if the fqdn isn't resolvable, try the short hostname
        ...
        print(f"[Rank 0] Determined master hostname: {hostname}, resolved to IPv4: {ip_address}")

    # Synchronize all processes to make sure the file is written.
    time.sleep(2) # A slightly longer sleep for filesystem robustness

    with open(temp_file_path, "r") as f:
        master_addr = f.read().strip()

    master_port = "12355"
    init_method_url = f"tcp://{master_addr}:{master_port}"
    
    print(f"[Rank {rank}] Initializing process group with method: {init_method_url}")
    
    # Initialize using the explicit IPv4 address.
...
def load_checkpoint(model, optimizer, scaler, device, checkpoint_dir="checkpoints"):
    ...
    if os.path.isfile(checkpoint_path):
        print(f"Loading checkpoint from {checkpoint_path} to {map_location}")
        try:
            ...
            # The model is loaded *before* DDP wrapping, so it won't have the 'module.' prefix.
            # We must strip the 'module.' prefix from the checkpoint if it exists.
...
def plot_residual_vs_sigma(mu, sigma, y_true, param_names, job_id):
    ...
    for i, name in enumerate(param_names):
        cal_factor = CALIBRATION_FACTORS.get(name, 1.0)
        plt.figure(figsize=(6, 5))
        plt.scatter(sigma[:, i]*cal_factor, residual[:, i], alpha=0.3, s=10)
        ...
        within_1sigma = ((mu[:, i] > y_true[:, i] - sigma[:, i]*cal_factor) & (mu[:, i] < y_true[:, i] + sigma[:, i]*cal_factor)).float().mean().item()
...
def evaluate_calibration_old(mu, sigma, y_true, param_names, job_id):
...
def plot_diagnostic_spectra(train_loader, num_samples, job_id, freq_axis):
    """
    Loads a few samples from the train_loader, computes the spatially-averaged 
    ...
    # Get the underlying FitsDataset object
    base_dataset = get_base_dataset(train_loader)
    
    # --- Step 1: Collect the first `num_samples` from the loader ---
    collected_samples = [] # Will store tuples of (cube, rms)
    loader_iter = iter(train_loader)
    while len(collected_samples) < num_samples:
        try:
            # The loader correctly returns only two items per sample
            batch_data, _ , batch_rms= next(loader_iter)
            
            for i in range(batch_data.size(0)):
                if len(collected_samples) < num_samples:
                    # Squeeze the channel dimension (dim=1) but keep the 3D cube
                    cube = batch_data[i].squeeze(0)
                    rms = batch_rms[i].item() # .item() to get the float value
                    collected_samples.append((cube, rms))

        except StopIteration:
            print(f"Warning: Reached end of dataloader. Only found {len(collected_samples)} samples for diagnostic plot.")
            break
    ...
    plot_dir = f"Diagnostic_Plots/{job_id}/"
    os.makedirs(plot_dir, exist_ok=True)

    for i, (spectrum_cube, noise_rms) in enumerate(collected_samples):
        # spectrum_cube has shape [channels, height, width]
        # Get the spatial dimensions
        height = spectrum_cube.shape[1]
        width = spectrum_cube.shape[2]

        # Extract the 1D spectrum from the central spatial pixel
        spectrum_1d = spectrum_cube[:, height // 2, width // 2].numpy()

        # --- SNR CALCULATION ---
        snr_text = "SNR: N/A" # Default text
        # We can only calculate SNR if noise was added and we have a frequency axis
        if noise_rms > 0 and freq_axis is not None:
            # Constants for the k=3 line
            C_KMS = 299792.458
            FREQ_K3_GHZ = 220.7090170
            VEL_WINDOW_KMS = 10 # +/- 10 km/s window to find the peak
            
            freq_window_ghz = FREQ_K3_GHZ * (VEL_WINDOW_KMS / C_KMS)
            k3_freq_min = FREQ_K3_GHZ - freq_window_ghz
            k3_freq_max = FREQ_K3_GHZ + freq_window_ghz
            ...
            if np.any(k3_mask):
                # Find the peak signal within the k=3 line window of the 1D spectrum
                signal_peak = np.max(spectrum_cube[k3_mask,:,:].numpy())
        ...
        # --- Plotting for this single sample ---
        ...
        # Visually indicate the masked ¹³CO region
        if freq_axis is not None:
            # Constants from your dataloader for visual consistency
            ...
            plt.legend()
        # --- ADD THE SNR TEXT TO THE PLOT ---
        plt.text(0.02, 0.95, snr_text, transform=plt.gca().transAxes, fontsize=12,
...
def train(model, train_loader, optimizer, criterion, device, scaler, use_mdn, model_params, enable_profiling=False, profile_steps=10):
    model.train()
    running_loss = 0.0

    model_params = model_params if model_params is not None else TARGET_PARAMETERS
        ...
        for batch_idx, (data, target,_) in enumerate(train_loader):
            ...
            with autocast(device_type='cuda', dtype=torch.float16):
                ...
                if use_mdn:
                    ...
                    output_dict = {}
                    for i, param_name in enumerate(model_params):
...
def test(model, test_loader, criterion, device, epoch, use_mdn, model_params, log_scale_params, job_id, original_dataset, calibration_factors):
    model.eval()
    test_loss = 0.0 # Total loss across all batches

    model_params = model_params if model_params is not None else TARGET_PARAMETERS
    log_scale_params = log_scale_params if log_scale_params is not None else args.log_scale_params
    job_id = job_id if job_id is not None else args.job_id

    all_preds, all_targets, all_sigmas = [], [], []
    loss_per_param = {param: [] for param in model_params}
    ...
    for i, current_param_name in enumerate(model_params):
        ...
        if use_mdn:
            ...
            # Get the calibration factor for this parameter, default to 1.0 if not yet calculated
            cal_factor = calibration_factors.get(current_param_name, 1.0)
            y_err = y_err * cal_factor  # Apply the calibration factor to the uncertainty

        plt.figure(figsize=(7, 7))  # Square aspect ratio

        # Log-log plot for selected parameters
        if current_param_name in log_scale_params:
            plt.xscale('log')
            plt.yscale('log')
        ...
        if use_mdn and "plummer_shape" in model_params and "p" in model_params and "rr" in model_params and current_param_name == "plummer_shape":
            print("Generating derived radius plot...")
            try:
                # Get the indices of the required parameters
                ps_idx = model_params.index("plummer_shape")
                p_idx = model_params.index("p")
                rr_idx = model_params.index("rr")
                ...
                # Scale sigma by the calibration factor if available
                cal_y = calibration_factors.get("plummer_shape", 1.0)
                sigma_y = sigma_y * cal_y  # Apply the calibration factor to the uncertainty
                
                p_pred = outputs_original[:, p_idx]
                sigma_p = sigmas_original[:, p_idx]
                # Scale sigma by the calibration factor if available
                cal_p = calibration_factors.get("p", 1.0)
                sigma_p = sigma_p * cal_p  # Apply the calibration factor to the uncertainty
...
def main(args):
    ...
    # 0) Nuke env-based rendezvous to prevent the default env:// path
    for k in ("MASTER_ADDR", "MASTER_PORT", "TORCHELASTIC_RENDEZVOUS_ENDPOINT"):
        os.environ.pop(k, None)
    os.environ.setdefault("GLOO_USE_IPV6", "0")
    os.environ.setdefault("TP_USE_IPV6", "0")
    os.environ.setdefault("C10D_USE_IPV6", "0")



    # --- DDP RANK/ENV RESOLUTION ---
    # Prefer PyTorch env (set by your srun block); fall back to SLURM_* if needed.
    ...
    # 2) Read rendezvous from CLI (NOT from env)
    master_addr = args.rdzv_addr
    master_port = int(args.rdzv_port)

    # 3) Validate IPv4 literal (defensive)
    ipaddress.IPv4Address(master_addr)  # raises if not IPv4

    # Basic sanity (fast to read in logs)
    if rank == 0:
        print(f"[DDP] MASTER {master_addr}:{master_port}  | WORLD_SIZE={world_size}")
    ...
    print("RDZV", master_addr, master_port, "RANK", rank, "WS", world_size, flush=True)


    # TCPStore signature on your system:
    # TCPStore(host_name, port, world_size=None, is_master=False,
    #          timeout=timedelta(seconds=300), wait_for_workers=True,
    #          multi_tenant=False, master_listen_fd=None, use_libuv=True)
    # TCPStore signature (positional), and disable libuv to avoid IPv6 paths
    store = TCPStore(
        master_addr,                          # host_name (IPv4 literal)
        master_port,                          # port (int)
        world_size,                           # world_size
        is_master,                            # is_master
        datetime.timedelta(minutes=30),       # timeout
        True,                                 # wait_for_workers
        False,                                # multi_tenant
        None,                                 # master_listen_fd
        False,                                # use_libuv = False
    )

    print(f"[Rank {rank}] init pg via TCPStore {master_addr}:{master_port}")
    dist.init_process_group(
        backend="nccl",
        store=store,
        rank=rank,
    ...
    t = torch.tensor([dist.get_rank()], device=f"cuda:{local_rank}", dtype=torch.int64)
    dist.all_reduce(t, op=dist.ReduceOp.SUM)
    if dist.get_rank() == 0:
        print(f"[DDP smoke] sum={t.item()} (expected {sum(range(dist.get_world_size()))})", flush=True)

    is_main_process = (rank == 0)

    # (optional but helpful on multi-node)
    if is_main_process:
        print(f"[DDP] Using device cuda:{local_rank} on rank {rank}")
    ...
    # --- Data Loading ---
    # NOTE: The full dataset is instantiated on every process, but the Sampler
    # ensures each process only loads its unique subset.
    

        # DDP CHANGE: The sampler will handle shuffling for DDP.
        # We pass the sampler object to the dataloader creation function.
        # You will need to modify `create_dataloaders` to accept a `train_sampler` argument.
    
    train_loader, test_loader, dataset, train_sampler = create_dataloaders(
        ...
        # --- Pass the DDP state directly to the function ---
        use_ddp=args.use_ddp,
        rank=rank,
        world_size=world_size,
        seed=42, # Use a fixed seed for reproducible splits
    )

    if is_main_process:
        print(f"Train dataset size: {len(train_loader.dataset)}")
        print(f"Test dataset size: {len(test_loader.dataset)}")
        # Diagnostic plotting is done only by the main process
    ...
    # --- after building loaders ---
    steps_local = len(train_loader)
    t = torch.tensor([steps_local], device=device, dtype=torch.int64)
    dist.all_reduce(t, op=dist.ReduceOp.MIN)
    if steps_local != t.item():
        raise RuntimeError(f"[Rank {rank}] Dataloader length mismatch across ranks "
                        f"(this={steps_local}, min={t.item()})")
    ...
    model = generate_2d_model(
        config_name=config_name, # Or your logic to select it
        use_batchnorm=False,
        target_params_list=TARGET_PARAMETERS,
        n_outputs=num_outputs,
        use_attention_heads=args.use_attention_heads,
        use_mdn=args.use_mdn
    )

    # DDP CHANGE: Move model to the assigned GPU *before* wrapping with DDP.
    model.to(device)

    if args.use_ddp:
        # DDP CHANGE: Wrap the model with DistributedDataParallel.
        # This replaces `nn.DataParallel`.
        model = DDP(
            model.to(device),
            device_ids=[local_rank],
            output_device=local_rank,
            broadcast_buffers=False,
            find_unused_parameters=False,   # start with True; switch off later if graph is static
            gradient_as_bucket_view=True,
            bucket_cap_mb=50,              # smaller buckets -> easier diagnostics
        )
    elif torch.cuda.device_count() > 1:
        # Your original DataParallel logic for the non-DDP multi-GPU case.
        if is_main_process:
    ...
    dist.barrier()  # Ensure all processes reach this point before starting training
    ...
    # DDP CHANGE: Load checkpoint onto the correct device map.
    # The `load_checkpoint` function might need a `map_location` argument.
    start_epoch, best_loss, _, _ = load_checkpoint(
        model.module if args.use_ddp else model, # Access underlying model
        optimizer,
        scaler,
        device,
        checkpoint_load_dir
    ...
    s = torch.tensor([rank], device=device, dtype=torch.int64)
    dist.all_reduce(s, op=dist.ReduceOp.SUM)
    if rank == 0:
        print(f"[DDP smoke] sum={s.item()} expected={sum(range(world_size))}")
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        # DDP CHANGE: Set the epoch for the sampler.
        # This is crucial for ensuring proper shuffling year over year.
        if args.use_ddp:
            train_sampler.set_epoch(epoch)

        # Each process runs the training function on its subset of data.
        train_loss = train(model, train_loader, optimizer, criterion, device, scaler, use_mdn=args.use_mdn, model_params=args.model_params)

        # DDP CHANGE: Aggregate the training loss from all processes.
        if args.use_ddp:
            loss_tensor = torch.tensor(train_loss).to(device)
            dist.all_reduce(loss_tensor, op=dist.ReduceOp.SUM)
            aggregated_train_loss = loss_tensor.item() / world_size
        else:
            aggregated_train_loss = train_loss
        
        train_losses.append(aggregated_train_loss)

        # --- Evaluation and Checkpointing (Main Process Only) ---
        if is_main_process:
            # The 'module' attribute is used to access the original model inside the DDP wrapper.
            model_to_test = model.module if args.use_ddp else model
            
            test_loss, preds, targets, loss_per_param, sigmas = test(
                model_to_test, test_loader, criterion, device, epoch,
                use_mdn=args.use_mdn, model_params=args.model_params,
                log_scale_params=args.log_scale_params,
                job_id=args.job_id,
            ...
            # Log the aggregated training loss
            print(f"Train Loss: {aggregated_train_loss:.4f} | Validation Loss: {test_loss:.4f}")
            ...
            # All plotting and saving is done here
            save_training_plots(
                train_losses=np.array(train_losses),
                test_losses=np.array(test_losses),
                loss_per_param_history=loss_per_param_history,
                labels=args.model_params,
                job_id=args.job_id,
                epoch=epoch,
            ...
            if (epoch + 1) % checkpoint_interval == 0 or (epoch + 1) == max_epochs:
                # --- CALIBRATION LOGIC (This is fine as is, since it runs on rank 0) ---
                if args.use_mdn:
                ...
                if test_loss < best_loss:
                    print(f"New best validation loss: {test_loss:.4f} (previous: {best_loss:.4f})")
                    
                    # DDP CHANGE: Unwrap the model before saving.
                    # This ensures you save the actual model weights, not the DDP wrapper.
                    model_to_save = model.module if args.use_ddp else model
                    
                    save_checkpoint(epoch, model_to_save, optimizer, scaler, train_losses, test_losses, test_loss, checkpoint_save_dir)
                    print(f"Checkpoint saved at epoch {epoch+1}")
                    best_loss = test_loss
                else:
                    print(f"No improvement in validation loss: {test_loss:.4f} (best: {best_loss:.4f})")
                    print(f"Checkpoint not saved at epoch {epoch+1}, continuing training...")
            
            # Final evaluation and CSV saving at the end of training
            if epoch + 1 == max_epochs:
                ...
                if args.use_mdn:
                    ...
                    evaluate_calibration(outputs_original, sigmas_original, targets_original, args.model_params, args.job_id, epoch)
        ...
        dist.barrier() # Synchronize all processes before next epoch

    # DDP CHANGE: Clean up the process group.
    if args.use_ddp:
        dist.destroy_process_group()
    

if __name__ == "__main__":
    # DDP CHANGE: The entry point now primarily handles argument parsing
    # and launching the training processes.
    import argparse
    import torch
    import torch.multiprocessing as mp

    # --- Argument Parsing (Unchanged, with one addition) ---
    parser = argparse.ArgumentParser(description="ResNet Training Script with DDP Support")
    parser.add_argument("--rdzv-addr", type=str, required=True)
    parser.add_argument("--rdzv-port", type=int, required=True)
    ...
    parser.add_argument("--job_id", type=str, default="ddp_run")
    ...
    parser.add_argument('--use_attention_heads', type=lambda x: x.lower() == 'true', default=False, help="Use attention heads in the model")
    parser.add_argument('--use_mdn', type=lambda x: x.lower() == 'true', default=True, help="Use MDN output heads for uncertainty estimation")
    ...
    parser.add_argument("--cauchy-mu", type=float, default=0.003, help="Location parameter for Cauchy noise RMS in Jy/beam.")
    parser.add_argument("--cauchy-sigma", type=float, default=0.0032, help="Scale parameter for Cauchy noise RMS in Jy/beam.")
    parser.add_argument("--cauchy-threshold", type=float, default=0.07, help="Maximum allowed noise RMS in Jy/beam.")
    
    # --- DDP Control Arguments ---
    parser.add_argument('--use-ddp', type=lambda x: x.lower() == 'true', default=True, help='Enable Distributed Data Parallel training.')
    parser.add_argument('--seed', type=int, default=42, help='Random seed for reproducible train/test splits and data sampling.')
    ...
    if args.use_ddp:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("DDP requires CUDA, but it is not available.")
        
        main(args=args)
    else:
        # --- STANDARD (NON-DDP) LAUNCH LOGIC ---
        print("Trying to run in standard single-GPU or CPU mode.")
        # Call the main function directly for a standard run.
