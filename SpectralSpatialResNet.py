...
def test(model, test_loader, criterion, device):
    ...
    outputs_original = test_loader.dataset.inverse_transform_labels(all_preds)
    targets_original = test_loader.dataset.inverse_transform_labels(all_targets)
...
if __name__ == "__main__":
    ...
    os.makedirs(f"ResNetPlots/{files_per_job}_files", exist_ok=True)
    # Create job ID folder
    if args.job_id is not None:
        os.makedirs(f"ResNetPlots/{files_per_job}_files/{args.job_id}", exist_ok=True)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            os.makedirs(f"ResNetPlots/{files_per_job}_files/{args.job_id}/Epochs", exist_ok=True)
