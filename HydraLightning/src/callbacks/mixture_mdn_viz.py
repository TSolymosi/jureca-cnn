...
class MixtureMDNVizCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        # Retrieve the cache. We expect the LightningModule to have already
        # gathered and concatenated the tensors from all DDP processes.
        cache = getattr(pl_module, "_val_cache", {})
        
        # --- THIS IS THE SIMPLIFIED LOGIC ---
        # Fetch the final, concatenated tensors directly.
        # No need to check for lists, as the module's hook has handled it.
        y         = cache.get("targets", None)
        ...
        # Now, the check for None is sufficient. If the data exists, it's a tensor.
        if y is None or pi_logits is None or mu_all is None:
            print("[MixtureMDNVizCallback] No cached & concatenated mixture data found. Skipping viz.", flush=True)
            return
        
        # --- END OF SIMPLIFIED LOGIC ---
        
        # Now we can safely check the shapes and proceed
        print(f"[DEBUG] MixtureMDNVizCallback: Final tensor shapes -> y: {y.shape}, pi_logits: {pi_logits.shape}, mu_all: {mu_all.shape}, L_all: {L_all.shape if L_all is not None else None}, sigma_all: {sigma_all.shape if sigma_all is not None else None}", flush=True)
