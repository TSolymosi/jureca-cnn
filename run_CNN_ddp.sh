...
# This is the total number of GPUs you requested for the node (e.g., 4).
GPUS_PER_NODE=4 
# This is the total number of CPU cores you requested for the job.
TOTAL_CPUS=64
# Calculate CPUs per GPU task. Use integer division.
...
# --- CRITICAL NETWORK SETUP (From the working example) ---
# 1. Get the primary node's hostname from SLURM
export MASTER_ADDR=\"\$(scontrol show hostnames \"\$SLURM_JOB_NODELIST\" | head -n 1)\"
# 2. Append the InfiniBand suffix 'i' required for JURECA-DC
export MASTER_ADDR=\"\${MASTER_ADDR}i\"
# 3. Explicitly tell NCCL to use the InfiniBand interface
export NCCL_SOCKET_IFNAME=ib0
# 4. Use a fixed, high-numbered port, just like the example
export MASTER_PORT=54123

echo \"Master Address set to: \${MASTER_ADDR}\"
echo \"Master Port set to: \${MASTER_PORT}\"
echo \"NCCL Interface set to: \${NCCL_SOCKET_IFNAME}\"

echo 'Job ID: ${JOB_ID}'
echo 'Continuing training with job ID: ${TRAINING_JOB_ID}'
echo \"[Task \${SLURM_PROCID}] Executing Python script: ${SCRIPT_PATH}\"

# --- torchrun now uses the environment variables we just set ---
...
echo \"[Task \${SLURM_PROCID}] Python script finished.\"
