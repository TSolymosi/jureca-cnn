...
import sys
sys.path.insert(0, "/p/scratch/westai0043/CNN_timon/Hydra_Lightning/")
...
def gaussian_nll_loss_dict(output_dict, target_tensor, std_weights=None, sigma_floor=1e-4, reduction="mean"):
    """
    Args:
        output_dict: dict like {param_name: {"mu": [B] or [B,1], "sigma": [B] or [B,1]}}
                     IMPORTANT: 'sigma' is already positive from the MDN head (e.g., Softplus).
        target_tensor: [B, P] targets (same param order as output_dict iteration)
        std_weights: optional tensor/list of per-parameter stds to weight loss by 1/std^2
        sigma_floor: numerical floor to avoid zero variance
        reduction: "mean" or "none"
    Returns:
        loss: scalar (if reduction="mean") or [B] (if "none")
        per_param_mean: dict of scalar means per parameter (for logging)
    """
    assert isinstance(output_dict, dict) and len(output_dict) > 0
    B = target_tensor.size(0)

    losses = []
    per_param_mean = {}
    # keep deterministic param order
    names = list(output_dict.keys())

    for i, name in enumerate(names):
        mu = output_dict[name]["mu"].squeeze(-1)
        sigma = output_dict[name]["sigma"].squeeze(-1)  # already positive!
    ...
    if reduction == "none":
        return loss_mat.mean(dim=1), per_param_mean
    return loss_mat.mean(), per_param_mean
...
def generate_2d_model(config_name="s1_spec_conv_like", TARGET_PARAMETERS=["D", "L", "rr", "ro", "p", "Tlow", "NCH3CN", "plummer_shape"], **kwargs):
    """
