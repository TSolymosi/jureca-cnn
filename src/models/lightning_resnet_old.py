...
class LitResNetMDN(LightningModule):
    ...
    def on_fit_start(self):
        ...
        if self.trainer.is_global_zero:
            ...
            # Also print scaler stats aligned to names
    ...
    def _split_batch(self, batch):
        # Supports both 2-tuple (x, y) and 3-tuple (x, y, noise_rms)
    ...
    def _compute_loss(self, output, target, return_per_param: bool = False):
        ...
        # Compute NLL in full precision for stability (avoid fp16 underflow on σ)
        import torch as _torch
        with _torch.autocast(device_type="cuda", enabled=False):
            for i, name in enumerate(self.model_params):
                # shapes: output[i*2] and output[i*2+1] are [B, 1]; squeeze -> [B]
                mu    = output[i*2].squeeze(-1).float()      # [B], fp32
                sigma = output[i*2+1].squeeze(-1).float()    # [B], raw σ (can be <0)

                # Smooth, AMP-friendly variance: σ² + ε instead of clamping σ
                var = sigma.mul(sigma).add(1e-6)             # [B], strictly > 0, smooth grads

                y_i = target[:, i].float()                   # [B], fp32

                # per-sample NLL in fp32
                li = F.gaussian_nll_loss(mu, y_i, var, full=True, reduction="none")  # [B]

                # NO std reweighting here (keep parity with baseline)
                ...
                if sigma_reg:
                    ...
                    if w > 0:
                        # mild regularizer if you ever enable it
                        reg_term += w * _torch.mean(_torch.log(var))  # log(σ²) ~ 2*log|σ|
        # average over params and batch
    ...
    def training_step(self, batch, batch_idx):
        x, y, noise_rms = self._split_batch(batch)
        ...
        if batch_idx == 0 and self.trainer.is_global_zero:
            ...
            with torch.no_grad():
                ...
                for i, name in enumerate(self.model_params):
                    ...
                    y_i = y[:, i]
                    nll_i = F.gaussian_nll_loss(mu, y_i, sigma*sigma, full=True, reduction="none").mean()
